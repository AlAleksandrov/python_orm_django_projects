...
def filter_authors_by_nationalities(nationality) -> str:
    ...
    for a in authors:
        if not a.biography:
            result += [f"{a.first_name} {a.last_name}"]
        else:
            result += [f"{a.biography}"]
    return '\n'.join(result)
...
def filter_authors_by_birth_year(first_year, last_year) -> str:
    authors = Author.objects.filter(birth_date__year__gte=first_year, birth_date__year__lte=last_year).order_by('-birth_date')
...
def change_reviewer_name(old_name, new_name) -> QuerySet:
